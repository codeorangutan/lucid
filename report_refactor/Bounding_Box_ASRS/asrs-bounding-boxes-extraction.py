...
def extract_asrs_bounding_boxes(pdf_path):
    ...
    for i, page in enumerate(pdf.pages):
        ...
        if "Adult ADHD Self-Report Scale (ASRS-v1.1) Symptom Checklist" in text:
            asrs_page = page
            break
...
if __name__ == "__main__":
    pdf_path = "33957-20230729170840.pdf"
    df = extract_asrs_bounding_boxes(pdf_path)
    df.to_csv("asrs_bounding_boxes.csv", index=False)
    print(f"ASRS bounding boxes exported to asrs_bounding_boxes.csv")
