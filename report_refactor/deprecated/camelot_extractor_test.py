...
def identify_test_from_table(df, expected_tests):
    ...
    # Extract all column values (potential subtest names)
    all_values = []
    ...
    print(f"DEBUG: Table values: {all_values[:10]}...")
    ...
    for test_name in expected_tests:
        ...
        for value in all_values:
            ...
            # Check for partial match (subtest in value)
...
def extract_subtests_from_pdf(pdf_path, patient_id=None):
    ...
    for page_num in [1, 2, 3]:
        ...
        try:
            # Try lattice mode first with more flexible settings
            print("  Trying lattice mode with custom settings...")
            tables = camelot.read_pdf(
                ...
                line_tol=2  # More tolerance for line detection
            )
