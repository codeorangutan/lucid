...
def create_db(reset=False):
    ...
    # Check if the is_valid column exists in subtest_results table
    # If not, add it
    try:
        # Check if column exists
        cur.execute("SELECT is_valid FROM subtest_results LIMIT 1")
        cur.fetchone()  # This will raise an OperationalError if the column doesn't exist
    except sqlite3.OperationalError:
        # Column doesn't exist, add it
        print("Adding is_valid column to subtest_results table...")
        cur.execute("ALTER TABLE subtest_results ADD COLUMN is_valid INTEGER DEFAULT 1")
        conn.commit()
...
def extract_dsm_diagnosis(asrs_responses, patient_id):
    ...
    """
    try:
        # Import ASRS_DSM_mapper functionality
        ...
        print(f"[ERROR] Failed to extract DSM diagnosis: {e}")
        ...
        try:
            ...
            if cognitive_subtests:
                ...
                for pid, test, subtest, raw, std, pct in cognitive_subtests:
                    ...
                    if "Invalid" in subtest or "Possibly Invalid" in subtest:
                        is_valid = 0
        ...
        try:
            ...
            if cognitive_subtests:
                conn.executemany("""
                    ...
                    (patient_id, subtest_name, metric, score, standard_score, percentile, is_valid)
            ...
            # --- DSM Diagnosis ---
            if asrs:
                inattentive_met, hyperactive_met, diagnosis, dsm_criteria_data = extract_dsm_diagnosis(asrs, patient_id)
                insert_dsm_diagnosis(conn, patient_id, inattentive_met, hyperactive_met, diagnosis)
                insert_dsm_criteria_met(conn, patient_id, dsm_criteria_data)
                log_section_status(conn, patient_id, "DSM Diagnosis", "parsed")
                print(f"[INFO] DSM Diagnosis: {diagnosis} (Inattentive: {inattentive_met}/9, Hyperactive: {hyperactive_met}/9)")
            else:
                log_section_status(conn, patient_id, "DSM Diagnosis", "missing")
...
def parse_subtests_new(cognitive_text, patient_id):
    ...
    subtests = []
    current_test = None
    current_test_valid = True  # Track validity of current test
    ...
    for line in lines:
        ...
        if any(test in line for test in [
            ...
            # Check if test is marked as invalid
            current_test_valid = True  # Reset validity for new test
            if "Invalid" in line or "Possibly Invalid" in line:
                current_test_valid = False
                print(f"DEBUG: Found INVALID test section: {current_test}")
            else:
            ...
            if 3 <= len(numbers) <= 4:
                ...
                if metric and len(metric) > 3:
                    try:
                        ...
                        print(f"DEBUG: Found score - Test: {current_test}, Metric: {metric}, Score: {score}, Std: {std_score}, Perc: {percentile}, Valid: {current_test_valid}")
                        ...
                        subtests.append((
                            ...
                            std_score,
                            percentile,
                            1 if current_test_valid else 0  # is_valid flag (1 = valid, 0 = invalid)
                        ))
                    except (ValueError, IndexError) as e:
...
def insert_patient(conn, patient_id, test_date, age, language):
    """
    Insert or replace patient information in the database.
...
def insert_cognitive_scores(conn, scores):
    """
    Insert cognitive scores into the database.
    
    Args:
        conn: Database connection
        scores: List of tuples with score data
    """
    conn.executemany("""
...
def log_section_status(conn, patient_id, section, status):
    """
    Log the status of a processing section for a patient.
