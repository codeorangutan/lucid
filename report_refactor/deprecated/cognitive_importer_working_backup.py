...
def create_db(reset=False):
    ...
    cur.execute("""
    CREATE TABLE IF NOT EXISTS npq_responses (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    patient_id INTEGER,
    domain TEXT,
    question_number INTEGER,
...
def import_pdf_to_db(pdf_path):
    ...
    patient_id, age, test_date, language = parse_basic_info(text)
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        try:
            ...
            # Convert question_data to the expected format for npq_responses
            npq_responses = []
            for question_num, question_text, score, severity, domain in question_data:
                # We don't have domain information for questions in this approach
                # So we'll use a placeholder
                npq_responses.append((patient_id, domain, question_num, question_text, score, severity))
            
            print("NPQ scores (about to insert):", npq_scores)
            print("NPQ responses (about to insert):", len(npq_responses))
            ...
            if npq_responses:
                conn.executemany("""
                    INSERT INTO npq_responses (patient_id, domain, question_number, question, score, severity)
                    VALUES (?, ?, ?, ?, ?, ?)""", npq_responses)
...
def extract_npq_with_pymupdf(pdf_path, npq_pages_indices):
    """Extract NPQ data using PyMuPDF approach from pdf_debug2.py"""
    import fitz  # PyMuPDF
    ...
    # Key phrases that indicate question sections
    domain_questions_markers = [d + " Questions" for d in domains]
    
    # Pattern to match question-answer format
    question_pattern = re.compile(r'^(\d+)\s+(.*?)\s+(\d)\s*-\s+(.*)$')
    
    try:
        doc = fitz.open(pdf_path)
        current_domain = None
        
        # Process each NPQ page
        for page_idx in npq_pages_indices:
            print(f"[DEBUG] Processing NPQ page {page_idx+1} with PyMuPDF")
            page = doc[page_idx]
            
            # Extract blocks and sort them top-down, left-right
            blocks = page.get_text("blocks")
            blocks = sorted(blocks, key=lambda b: (b[1], b[0]))  # top-down, left-right
            ...
            for block in blocks:
                ...
                for line in block_text.splitlines():
                    ...
                    # Check if this line indicates a new question section
                    for domain_marker in domain_questions_markers:
                        if domain_marker in line:
                            current_domain = domain_marker.replace(" Questions", "")
                            print(f"[DEBUG] Found question section: {current_domain}")
                            break
                    ...
                    if match and current_domain:
                        try:
                            ...
                            print(f"[DEBUG] Found question: {qn}. {qtext} -> {score} - {severity}")
                            question_data.append((qn, qtext, score, severity, current_domain))
                        except (ValueError, IndexError) as e:
                            print(f"[WARN] Failed to parse question line: {line} → {e}")
                            
                    # Look for domain scores in a format like "Attention 271 Severe"
                    for domain in domains:
                        if line.startswith(domain) and domain != current_domain:
                            # Try to extract score and severity
                            parts = line.split()
                            if len(parts) >= 3:
                                try:
                                    domain_name = parts[0]
                                    score_text = parts[1]
                                    severity_text = " ".join(parts[2:])
                                    
                                    # Validate score and severity
                                    if score_text.isdigit() and any(sev in severity_text for sev in ["Severe", "Moderate", "Mild", "Not a problem"]):
                                        score = int(score_text)
                                        severity = severity_text
                                        print(f"[DEBUG] Found domain: {domain_name}, score: {score}, severity: {severity}")
                                        domain_data.append((domain_name, score, severity))
                                except (ValueError, IndexError) as e:
                                    print(f"[WARN] Failed to parse domain score line: {line} → {e}")
        
        doc.close()
                                    
    except Exception as e:
        print(f"[ERROR] Error in PyMuPDF extraction: {e}")
    
    print(f"[DEBUG] Total found: {len(domain_data)} domains, {len(question_data)} questions")
    return domain_data, question_data
...
def extract_npq_table(pdf_path):
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        domain_data = extract_npq_with_bounding_boxes(pdf, npq_pages)
        
        # Now try to get question data using PyMuPDF approach
        question_data = []
        domain_data, question_data = extract_npq_with_pymupdf(pdf_path, npq_pages)
