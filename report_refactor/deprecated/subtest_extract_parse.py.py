...
# --- Function to Extract Text from Debug File Format ---

def extract_lines_from_debug(filename):
    """
    Reads the debug file, extracts the text content after the 'Line X: ' prefix,
    skips page markers, and returns a list of cleaned lines.
    """
    extracted_lines = []
    try:
        with open(filename, 'r', encoding='utf-8') as f:
            for line_num, line in enumerate(f):
                # Skip page marker lines entirely
                if "=== PAGE" in line and "TEXT CONTENT ===" in line:
                    continue

                # Use regex to find the content after "Line <number>: "
                match = re.match(r"Line \d+:\s?(.*)", line)
                if match:
                    content = match.group(1).rstrip() # Remove trailing whitespace only
                    extracted_lines.append(content)
                else:
                    # Handle lines that might not have the prefix but could still be content
                    if re.match(r"Line \d+:\s*$", line):
                         extracted_lines.append("")

    except FileNotFoundError:
        print(f"Error: Debug file '{filename}' not found.", file=sys.stderr)
        return None
    except Exception as e:
        print(f"An error occurred reading debug file '{filename}': {e}", file=sys.stderr)
        return None
    print(f"Extracted {len(extracted_lines)} lines from {filename}")
...
def parse_subtest_data_from_vbm(lines):
    ...
    print(f"\nParsing {len(lines)} extracted lines, starting from '{first_test_name}'...")
        ...
        if parsing_data:
            ...
            if measure:
                ...
                def safe_convert(value_str):
                    if value_str in ['-', 'NA']: return value_str
                ...
                if not is_duplicate:
                    ...
                    # print(f"Line {i}: Added data row: {measure}" + (f" (Desc: {description_part[:20]}...)" if description_part else "")) # Less verbose
                    ...
                    if description_part:
                        ...
                        continue
                # else:
                    # print(f"Line {i}: Skipping potential duplicate row: {measure}")
...
def main():
    if len(sys.argv) != 2:
        print(f"Usage: python {os.path.basename(__file__)} <debug_lines_file.txt>")
        sys.exit(1)

    debug_file = sys.argv[1]
    if not os.path.exists(debug_file):
        print(f"Error: Input file '{debug_file}' not found")
        sys.exit(1)

    # 1. Extract lines from the debug file
    extracted_lines = extract_lines_from_debug(debug_file)

    if extracted_lines is None:
        print("Failed to extract lines from debug file. Exiting.")
        sys.exit(1)

    # 2. Parse the extracted lines, starting from VBM
    results = parse_subtest_data_from_vbm(extracted_lines)

    # 3. Save results to JSON
    os.makedirs('parser', exist_ok=True)
    output_filename = os.path.basename(debug_file).replace('.txt', '_subtests_parsed.json') # Changed output name
