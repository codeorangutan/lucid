...
import fitz
import csv # PyMuPDF
from asrs_dsm_mapper import is_met, DSM5_ASRS_MAPPING, RESPONSE_SCORES
...
def create_db(reset=False):
    ...
    cur.execute("""
    CREATE TABLE IF NOT EXISTS asrs_dsm_criteria (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        patient_id INTEGER,
        dsm_criterion TEXT,
        asrs_question TEXT,
...
def import_pdf_to_db(pdf_path):
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        try:
            ...
            # --- ASRS DSM Diagnosis ---
            if asrs:
                asrs_dsm_diagnosis = parse_asrs_dsm_diagnosis(patient_id, conn)
                if asrs_dsm_diagnosis:
                    conn.execute("""
                        INSERT INTO asrs_dsm_diagnosis 
                        (patient_id, inattentive_criteria_met, hyperactive_criteria_met, diagnosis) 
                        VALUES (?, ?, ?, ?)
                    """, asrs_dsm_diagnosis)
                    log_section_status(conn, patient_id, "ASRS DSM Diagnosis", "parsed")
                else:
                    log_section_status(conn, patient_id, "ASRS DSM Diagnosis", "missing")
...
def parse_asrs_dsm_diagnosis(patient_id, conn):
    """
    Process ASRS responses for a patient and determine DSM-5 diagnosis.
    ...
    Returns:
        tuple: (patient_id, inattentive_criteria_met, hyperactive_criteria_met, diagnosis)
    """
    # Retrieve ASRS responses from the database
    cursor = conn.cursor()
    cursor.execute(
        "SELECT question_number, response FROM asrs_responses WHERE patient_id = ?",
        (patient_id,)
    )
    asrs_responses = {row[0]: row[1] for row in cursor.fetchall()}
    
    if not asrs_responses:
        print(f"[WARN] No ASRS responses found for patient {patient_id}")
        return None
    ...
    # Process Criterion A (Inattention, first 9 items)
    for i in range(9):
    ...
    # Process Criterion B (Hyperactivity/Impulsivity, last 9 items)
    for i in range(9, 18):
        dsm_crit, asrs_text, q_num = DSM5_ASRS_MAPPING[i]
        resp = asrs_responses.get(q_num, "N/A")
        criterion_met = is_met(resp, q_num)
    ...
    # Insert criteria results into database
    cursor.executemany(
        """INSERT INTO asrs_dsm_criteria 
           (patient_id, dsm_criterion, asrs_question, question_number, response, is_met, criterion_type)
           VALUES (?, ?, ?, ?, ?, ?, ?)""",
        criteria_results
    )
    
    return (patient_id, met_inattentive, met_hyperactive, diagnosis)
...
def parse_asrs_dsm_diagnosis(text, patient_id):
    """
    Parse ASRS DSM-5 diagnosis information from text.
    
    Returns:
        tuple: (patient_id, inattentive_criteria_met, hyperactive_criteria_met, diagnosis)
    """
    diagnosis = None
    inattentive_criteria_met = None
    hyperactive_criteria_met = None
    
    # Only match lines for DSM-5 diagnosis
    pattern = re.findall(r"DSM-5 Diagnosis:\s*(.+?)\s*Inattentive Criteria Met:\s*(\d+)\s*Hyperactive Criteria Met:\s*(\d+)", text)
    
    if pattern:
        diagnosis, inattentive_criteria_met, hyperactive_criteria_met = pattern[0]
        diagnosis = diagnosis.strip()
        inattentive_criteria_met = int(inattentive_criteria_met)
        hyperactive_criteria_met = int(hyperactive_criteria_met)
    
    return (patient_id, inattentive_criteria_met, hyperactive_criteria_met, diagnosis)
