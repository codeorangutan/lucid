...
def import_pdf_to_db(pdf_path):
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        try:
            ...
            # --- Epworth ---
            epworth_data = parse_epworth(text, patient_id)
            if epworth_data and epworth_data[1]:  # Check if we have Epworth responses
                total_score_data, responses = epworth_data
                # Insert total score
                if total_score_data[0] is not None:  # Check if total_score is not None
                    total_score, interpretation = total_score_data
                ...
                conn.executemany("INSERT INTO epworth_scores (patient_id, question_number, situation, score, description) VALUES (?, ?, ?, ?, ?)", 
                                responses)
                log_section_status(conn, patient_id, "Epworth", "parsed")
            else:
                log_section_status(conn, patient_id, "Epworth", "missing")
            ...
            if npq_scores:
                ...
                log_section_status(conn, patient_id, "NPQ", "parsed")
            else:
                log_section_status(conn, patient_id, "NPQ", "missing")
