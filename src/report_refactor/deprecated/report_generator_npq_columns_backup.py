...
def create_npq_section(data):
    ...
    def section_block(title, domains):
        rows = [(title,)]  # Remove empty columns from section title
        for domain in domains:
            for row in npq_scores:
                if row[1].lower() == domain.lower():
                    ...
                    rows.append((domain, score, severity))  # Remove description column
                    break
        return rows
    ...
    table = Table(full_table, colWidths=[200, 100, 150])
    ...
    if npq_questions:
        elements.append(PageBreak())  # Add page break before detailed responses
        ...
        for q in npq_questions:
            ...
            score = q[5]
            severity = q[6]
            grouped[domain].append((question_text, score, severity))  # Remove question number
        
        # Sort domains alphabetically for consistent presentation
        domains = sorted(grouped.keys())
        
        # Create table with headers
        response_rows = [("Question", "Score", "Severity")]  # Remove Domain column
        
        # Add each domain and its questions
        for domain in domains:
            # Add domain header
            response_rows.append((domain, '', ''))  # Simplified header row
        ...
        # Add colors for severity and domain headers
        row_idx = 0
        for row in response_rows:
            if row[1] == '' and row[2] == '':  # Domain header
                response_table_style.add('SPAN', (0, row_idx), (-1, row_idx))
                response_table_style.add('BACKGROUND', (0, row_idx), (-1, row_idx), colors.lightgrey)
                response_table_style.add('TEXTCOLOR', (0, row_idx), (-1, row_idx), colors.darkblue)
        ...
        response_table = Table(response_rows, colWidths=[350, 50, 100])
